 ...
 * `supabase/migrations` — not a `packages/db/drizzle` of its own — and
 * `migrations.prefix: 'supabase'` names files the way the Supabase CLI
 * expects (`<timestamp>_<slug>.sql`, timestamp = literal apply order).
 *
 * Imports nothing from `src/`: drizzle-kit loads this in its own esbuild
 * sandbox, and pulling in `env.ts` would make schema generation require a
 * valid `DATABASE_URL`. `drizzle-kit generate` never opens a connection — it
 * diffs `schema.ts` against `supabase/migrations`'s snapshots — but
 * `dbCredentials.url` is still declared, matching the shape drizzle-kit
 * expects for the `postgresql` dialect.
 */
